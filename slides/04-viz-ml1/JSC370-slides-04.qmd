...
- **Nonlinear**: each $(f_j(\cdot))$ is a flexible curve (often a spline basis)
...
* Method chaining passes the result of one operation to the next method call.
* We can think of it as a sequence of transformations, similar to R's pipe.
...
Merge flight routes with geospatial information (latitude and longitude)
...
* Goal: estimate a smooth function $f(s)$ that captures the trend
* Benefits:
...
$$B_{ij} = b_j(t_i), \quad S_{jk} = \int b_j''(t) b_k''(t) \, dt$$

The optimization problem becomes:

$$\min_\beta \; (y - B\beta)^\top (y - B\beta) + \lambda \beta^\top S \beta$$
...
|------|-------------|
| **Cubic splines** | Piecewise cubic polynomials joined smoothly |
| **Cyclic splines** | Cubic splines with connected ends (for periodic data) |
| **B-splines** | Basis splines with other polynomial orders |
| **Cardinal splines** | Knot placement is always a certain distance apart |
| **Natural splines** | Linear constraints at boundaries |
| **Smoothing splines** | Knots at every point, controlled by $\lambda$ |

---

## From Splines to GAMs

A **Generalized Additive Model (GAM)** extends this idea:

$$y = \beta_0 + f_1(x_1) + f_2(x_2) + \ldots + f_p(x_p) + \epsilon$$

Where each $f_j$ is a **smooth function** (typically a spline).

Key advantages:

* Each predictor can have its own non-linear relationship
* The effects are **additive** (easy to interpret)
* **Automatic smoothness selection** via penalization
...
| **Basis functions** | Building blocks combined to form the smooth |
| **Penalization** | Controls wiggliness, prevents overfitting |
...
2. **Polynomial regression** allows curves but has limitations
3. **Splines** are piecewise polynomials joined at knots
4. **GAMs** use splines to model each predictor flexibly
5. **geom_smooth()** uses these techniques under the hood
6. In Python: use `pygam` or `statsmodels` for GAM fitting
