...
- Our dataset has over than 450000 observations

- We can see that see that almost all our data is number, besides of MCI_Category, Offence, Location_Type, and Neighbourhood. All of this are categorical type.
...
```{python}
data.loc[data["OCC_YEAR"].isna(), :].head(4)
```
...
- Based on the number of observation of MCI Category, the most frequent category is Assault with 242671 observations.
...
- Within Assault Category, the top 3 most frequent are Assault, Assault with Weapon and Assault Bodily Harm. We will pick Assault category that has 166524 observations.
...
- We can see that in the pearson residual plot that there are lot of big value (bigger than 2). While by definition, it's classified as outlier, these situation suggest poor fit of the model. 
...
- First, we need to see that every point is placed on road intersection. This is intentional by the provider to keep privacy. They place the point to the nearest road intersection

- We can see there are lot of observations in Harbour Front. It may make sense since the place is very crowded, especially in national holiday, such as New Year and Christmas. 

- If we looks closer, we can see that many observations form a grid structure like a TTC path. It's maybe interesting to lay out TTC path in top of the map to see whether many of the assault happens in TTC.