...
- We can see that the top 3 of the medical specialties is Surgery, Consultant (Physician) and Cardiovascular

- Besides, we can see that the number of Surgery Specialty is doubled every other categories While the other categories is not so much different to each other.
...
- We can see that most of the top 20 most common words are stopwords or even punctuation (',', '.', ':'). Therefore, this doesn't give us any significant information about the data. 
...
- From the wordcloud, we can see that the words appear more useful. However, some of the words still heavily depended on the context, i.e left, right, weithout

- Some words also appear, such as anesthesia, history, anterior, diagnosis, and incision. Based on here, we can safely guess that our text related to medical record, surgery, and hospital. 
...
# Add custom stopwords
additional_stopwords = {"room", "normal", "left", "right"}  # Hint: add 3-4 clinical filler words
...
- There is 806 clinical stopwords in total. We can see that some of them can still be considered as stopwords in general and some of them are clinical terms

- From the graph, we can see that some of the previous most common words are still appear as common words in here. Words like history, procedure, pain, anesthesia, incision are still appeared. It suggests that these words aren't considered as clinical stopwords and carry some meaning.
...
- We can see that some of the bigram are very usual in medical records. For example, "blood pressure", "physical examination", "incision made", "postoperative diagnosis", and "blood loss". These bigrams even suggest more that the text we are analysing are medical record that heavily related with surgery (Operation). It's strengthen since the most popular specialties is "Surgery"
...
- Using "Diagnosis" as our common word, the word before diagnosis is very centered on "preoperative" and "postoperative". It's suggest that most part of the text are related with surgery and operation. 

- The word after "Diagnosis" are more variety, such as acute, bilateral, procedure. However, some of the them  are synonym, such as acute, severe, and chronic. We can also see some disease or clinical word, such as cataract, carpal, cervical and renal. Notice that some of these words are related with surgery, such as surgery for cataract, cervical and renal (kidney).

- We can see that some of the word combinations are not make sense. It may happens if "Diagnosis" is the first or the last word in a sentence. 
...
def get_sentence_stats(text):
    ...
    sentences =  nize(text)  # Hint: sentence tokenizer
    ...
    return {'num_sentences': len(sentence_lengths), 'avg_sentence_length': avg_length}  # Hint: use computed values
...
- In general, the average sentence length is around 20. Grouping by speciality, the average sentence length is also around 20 - 25. 

- Speciality with the highest average sentence length is Speech - Language
...
n_topics = 3  # Hint: number of topics
...
- It seems the topic is clearer when there are 3 topics. In 3 topics case, we can guess that the first topic is medical record and the third is related with circulatory. We can say that the second topic is related with surgery.

- The 5 topics can be guessed as bone related, skin surgery, cardiovascular and circulatory, medical record, and lower-body surgery. While it's still make senses, but the boundary between topic is less defined. 
...
- THe crosstab results is very aligned with what we found in the previous topic analysis.

- The first topic is dominated by Consult - History and Phy specialty, suggesting that it's very related with past medical record. The second topic is largely dominated by Surgery, suggesting that this topic is very related with surgery. 

- The third topic is more uniform across specialties. However, we can see that the number of Cardiovascular/Pulmonary in this topic is more than in the other topic. It slightly suggest that this topic is "more related" to Cardivascular/circulatory that the other topics.


### Github Repo:
https://github.com/PatuanPurba/JSC370-2026.git