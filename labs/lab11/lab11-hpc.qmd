...
Yes, our program use 1 CPU cores as we requested on lab11.sh
...
|--------|----------------|----------------|------------------|---------|
| `flu_1core.sh` | 1 | 3.03s| 2.58s | 1.18x | 
| `flu_4core.sh` | 4 | 2.74s | 0.92s | 2.99x |

*Is the speedup on the cluster similar to what you observed on your laptop in Lab 10? What factors might cause differences?*

No, it's much slower with only 3x while in local, it can get into 30x speed up. One of the factor is because different type Hardware (CPU/Processor) which lead to different baseline time. Second, we have different task, which may affect the degree of independence of subtasks. Besides, we may have different overhead cost of parallelization for local and cluster. 
...
|---------|-------------|------------------------|-----|
| 0 | 0% (0/900) | 45.85 | 7.2 |
| 1 | 25% (225/900) | 44.48 | 6.8 |
| 2 | 50% (450/900) | 40.67 | 6.45 |
| 3 | 75% (675/900) | 15.59 | 6.37 |
| 4 | 100% (900/900) | 1.21 | 1.1 |

*Describe the trend. At what vaccination level do deaths drop most sharply?*
As the vaccination rate increase, the mean_decesased decrease but the standard deviation fairly same (Except for 100% Vaccination). 

In term of absolute unit, the most death drop is from 50% into 75% (25 units). However, in term of percentages, the most significant death drop is between 75% - 100% (Drop by 90%)
...
The impact of additional unit of vaccination rate is not linear, with the impact is hgiher as the level is higher (Percentagely). It's because when the vaccination rate is higher, then overtime, the infected person become exponentially less which makes the overall spreading rate decrease too. This is called about herd immunity where most of the person is vaccinated, non-vaccinated person has less source of infection (reducing risk of infected on non-vaccinated person)
---
...
*Your answers here.*
1. SLURM is the cluster’s resource manager and scheduler. Its role is to decide who gets access to compute nodes, how much CPU and memory each job can use, and when jobs should run so that many users can share the cluster fairly and safely. We can't just SSH directly to a compute node because compute nodes shouldn't be used freely like personal machines. Without SLURM as manager and scheduler, it's harder to enforce resource limit and users can interfere with each other's jobs. 

2. A SLURM job array is a way to submit many similar jobs at once using one batch script. Each task in the array gets its own array index, which can be used to choose a different parameter, input file, or random seed. It become more convenient if each jobs has similar structure and only differs in small parameter or input value (That can be made into a list)

3. It's very different, with speedup in local is around 20x - 40x and speedup in cluster is only around 3x. Hardware difference, such as RAM, memory, CPU, may explain the gap since it can lead to different baseline time for Serial and Parallel

4. In 3b, we can see that a run with 1 processor takes around 3s. Since we ran 5 similar tasks in SLURM array, then by extrapolating, it takes around 15s to run all the tasks serially. For job array, since all the tasks run simultaneously, it only takes around 3s.